	...
	list_for_each_entry_safe(pkt, t, &packets_list, list) {
		...
		if (!ret) {
			...
			if (found) {
				...
				/*
				 * No pid to look up, and no pidfd verdict either --
				 * a stale one is handled above, and it does apply to
				 * these packets: scm_set_cred() keeps the struct pid
				 * and derives the number from it with pid_vnr(),
				 * which yields 0 for a pid not nameable in the pid
				 * namespace we peeked the queue from. So a zero pid
				 * does not mean the skb carries no struct pid, and
				 * where a pidfd proves that pid dead we prefer a
				 * stand-in over a bare packet.
				 *
				 * What is left here is a packet we know nothing more
				 * about: restore it exactly as it was dumped.
				 */
			} else if (pkt->pidfd_probed) {
				/*
				 * The pidfd proved the sender is alive, yet it is
				 * outside the dumped tree, so it won't exist after
				 * restore. There is no faithful way to reattach its
				 * pid; drop the packet rather than mislabel it as
				 * stale.
				 */
				...
				/*
				 * No pidfd verdict (the receiver lacks SO_PASSPIDFD)
				 * and the sender is not in the dumped tree: keep the
				 * pre-existing SCM_CREDENTIALS behaviour and drop the
				 * packet.
				 */
